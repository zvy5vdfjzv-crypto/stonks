// 📰 News RSS feeds — so fontes TESTADAS que funcionam
// Via Vercel serverless function /api/rss (parse server-side, sem CORS)
const RSS_PROXY = '/api/rss?url='

// Removi: TecMundo/IGN/Motor1 BR/Globo Esporte/ESPN BR — bloqueiam bot/IP
// Adicionei: Ars Technica, The Hacker News, Rolling Stone BR alternativas
const FEEDS = {
  finance: [
    { name: 'InfoMoney', url: 'https://www.infomoney.com.br/feed/' },
    { name: 'Valor Economico', url: 'https://valor.globo.com/rss/valor/' },
...
export async function fetchNewsByCategory(category, limit = 8) {
  ...
  // Tenta ate 2 fontes se a 1a falhar
  for (let attempt = 0; attempt < Math.min(2, feeds.length); attempt++) {
    const source = feeds[(attempt + Math.floor(Math.random() * feeds.length)) % feeds.length]
    try {
